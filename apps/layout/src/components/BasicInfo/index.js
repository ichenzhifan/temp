...
class BasicInfo extends Component {
  ...
  getBasicInfoHtml() {
    ...
    let info = pick(setting, showPramKey);
    let basicInfo = [], i = 0;
    for (let index in info) {
      let item = info[index];
      item = item == null ? "" : item;
      index = customLabel[index] ? customLabel[index] : index.replace(/([A-Z]+)/g," $1");
      const itemHtml = `<div class="col-md-2 text-right">${index}:</div><div class="col-md-2 text-left"><strong id="detail-sheettype">${item}</strong></div>`;
      if (i % 3 === 0) {
