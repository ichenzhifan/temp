...
class TextEditor extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.opened!==nextProps.opened) {
      if (nextProps.textOptions) {
        ...
        const { ratio, oriHeight } = spreadOptions;
        ...
        // 计算出当前选择字号的px值
        const currentSize = Math.round(getPtByPx(fontSize*oriHeight));
  ...
  handleDoneClick() {
    ...
    const { oriHeight, oriWidth, ratio } = spreadOptions;
    const text = encodeURIComponent(this.state.text);
    let fontSize = getPxByPt(this.state.fontSize);
    const containerWidth = oriWidth;
    const containerHeight = oriHeight;
