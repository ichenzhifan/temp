...
export const onDrop = (that, event) => {
  ...
  let elementProps = getTransferData(event);
...
export const onRound = (that, event) => {
  ...
  // 更新值为round还是oval
  const updateValue = isInTypes(cameoShapeTypes.round, availCameoShapeTypes) ? cameoShapeTypes.round : cameoShapeTypes.oval;
  const { boundProjectActions } = actions;
  boundProjectActions.changeProjectSetting({
...
const updateElementBySize = (that, event) => {
  ...
  const elementOptions = merge({}, options, {
    id: element.get('id'),
    width: cameoSize.get('width') + cameoBleed.get('left') + cameoBleed.get('right'),
    height: cameoSize.get('height') + cameoBleed.get('top') + cameoBleed.get('bottom')
  });
...
export const handleCameoImageLoaded = (that) => {
  ...
  that.hideLoading();
  if (typeof doSnipping === 'function') doSnipping();
};
