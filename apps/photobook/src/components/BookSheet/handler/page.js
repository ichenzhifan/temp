 ...
 */
export const computedInnerSheet = (workspaceRatio, size, pages, pageIndex) => {
...
export const onDropPage = (that, targetPage, event) => {
  const ev = event || window.event;
  const drageData = getTransferData(ev);
  const dragePageId = drageData && drageData.pageId || '';
  const dropPageId = targetPage.get('id');

  // 如果拖拽的页面id为空或者拖拽的页面和目标页面的id相同.
  // 就不要做任何事.
  if (!dragePageId || dragePageId === dropPageId) {
    return;
  }
  ...
  boundProjectActions.movePageBefore(dragePageId, dropPageId);
