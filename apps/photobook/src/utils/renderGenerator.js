 ...
 */
export const makeCoverImage = (coverType, targetSize, spainWidth, done) => {
  // 根据封面类型, 获取相应的原始素材.
  const material = getMaterialsByCoverType(coverType, true);
 ...
 * @param {function} done 合并完成后的回调.
 */
export const makeInnerImage = (coverType, targetSize, done) => {
  // 根据封面类型, 获取相应的原始素材.
  const material = getMaterialsByCoverType(coverType, false);
