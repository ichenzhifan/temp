...
import { pageStep, spreadTypes, elementTypes, coverTypes, pageTypes, smallViewWidthInArrangePages, productTypes, shadowBaseSize } from '../contants/strings';
import { shadowImages } from '../contants/material';
import { checkIsSetCoverAsInnerBg, checkIsSupportImageInCover, checkIsSupportHalfImageInCover, checkIsSupportFullImageInCover } from './cover';

/* -------------private methods----------------*/
/**
 * 根据产品的名称, 获取该产品对应的内页的shadow的图片信息.
 */
const getShadowImage = (settings) => {
  ...
  switch (product) {
    case productTypes.LF:
      {
        shadowImageObject = shadowImages.lfb;
        break;
      }
    case productTypes.FM:
      {
        shadowImageObject = shadowImages.fma;
        break;
      }
    case productTypes.PS:
      {
        shadowImageObject = shadowImages.pressbook;
        break;
      }
    default:
      {
...
const getShadowData = (pages, settings) => {
  let shadow;
  const shadowImageObject = getShadowImage(settings);
  ...
  if (pages.size === 1) {
    ...
    const middleImageWidth = (page.get('height') * shadowImageBaseSize.middle.width) / shadowImageBaseSize.middle.height;
    const middleImage = {
    ...
    const middleImageWidth = (page1.get('height') * shadowImageBaseSize.middle.width) / shadowImageBaseSize.middle.height;
    const middleImage = {
      width: middleImageWidth,
      height: page1.get('height'),
      top: 0,
...
const getPageNumberData = (summary, pages, pagination, totalOfPages) => {
  ...
  if (currentPage) {
    switch (currentPage.get('type')) {
      ...
      case pageTypes.full:
        {
          isActiveLeftPage = isActiveRightPage = true;
          break;
        }
  ...
  if (summary.get('isCover')) {
    ...
    leftPage = {
      text: 'Back cover',
      active: isSupportHalfImageInCover ? false : isActiveLeftPage,
      disable: isSupportHalfImageInCover,
    ...
    leftPage = {
      ...
      // pressbook的第一页为不可用.
      disable: isPressBook && (leftSurfaceIndex - 1) === 0 ? true : false
    };
    rightPage = {
      text: rightText,
      active: isActiveRightPage,
      ...
      // pressbook的第后一页为不可用.
      disable: isPressBook && (rightSurfaceIndex - 1) === totalOfPages -1 ? true : false
    };
  }
 ...
 */
export const checkIsEnablePage = (totalOfSheets, currentSheetIndex, currentPageIndex, productType, coverType, isCover) => {
  let enabled = true;
  ...
  if (!isCover) {
    ...
    if (productType === productTypes.PS &&
    ...
    if (!checkIsSupportImageInCover(coverType)) {
      ...
      if (checkIsSupportHalfImageInCover(coverType) &&
 ...
 */
const convertSheetPages = (allPages, firstPageIndex, secondPageIndex, productType, coverType, totalOfSheets, currentSheetIndex) => {
 ...
 */
export const computedRatioForSpecialView = (size, ratios, viewWith = 0) => {
  let coverWorkspace = 0;
  let innerWorkspace = 0;

  // 计算cover的相关ratio.
  if (size.coverSpreadSize && size.coverSpreadSize.width) {
    coverWorkspace = viewWith / size.coverSpreadSize.width;
  }

  // 计算inner的相关ratio.
  if (size.innerSpreadSize && size.innerSpreadSize.width) {
    innerWorkspace = viewWith / size.innerSpreadSize.width;
  }
 ...
 * - width: 基础宽 * 2 + 左右出血
 * - height: 基础高 + 上下出血
 * @param {object} bookBaseSize   - 内页的基础宽高. 结构为: {width, height}
 * @param {object} innerPageBleed - 内页的出血对象. 结构为: {left, right, top, bottom}
 */
export const getInnerSheetSize = (bookBaseSize, innerPageBleed) => {
  ...
  return {
    width: (bookBaseSize.width * 2) + innerPageBleedWidth,
    height: bookBaseSize.height + innerPageBleedHeight
  };
};

export const getInnerPageSize = (bookBaseSize, innerPageBleed) => {
  const innerPageBleedWidth = innerPageBleed.left + innerPageBleed.right;
  const innerPageBleedHeight = innerPageBleed.top + innerPageBleed.bottom;

  return {
    width: bookBaseSize.width + innerPageBleedWidth,
    height: bookBaseSize.height + innerPageBleedHeight
  };
};
 ...
 * - height: 基础高 + 延边 + 上下出血
 * @param {object} bookBaseSize       - 封面的基础宽高. 结构为: {width, height}
 ...
 */
export const getCoverSheetSize = (bookBaseSize,
  coverPageBleed,
  coverExpandingSize,
  spineWidth,
  addedSheetNumber = 0) => {
  const coverPageBleedWidth = coverPageBleed.left + coverPageBleed.right;
  const coverExpandingWidth = coverExpandingSize.left +
  ...
  const coverExpandingHeight = coverExpandingSize.top +
    coverExpandingSize.bottom;

  // spread的宽高.
  const width = (bookBaseSize.width * 2) + coverPageBleedWidth +
    coverExpandingWidth + totalSpineWidth;
  const height = bookBaseSize.height + coverPageBleedHeight +
...
export const getFrontCoverSize = (
  ...
  return {
    width: (bookBaseSize.width + coverPageBleedWidth +
      coverExpandingSize.right) - spineExpanding.expandingOverFrontcover,
    height: bookBaseSize.height + coverPageBleedHeight + coverExpandingHeight
  };
};
...
export const getBackCoverSize = (
  ...
  return {
    width: (bookBaseSize.width + coverPageBleedWidth +
      coverExpandingSize.left) - spineExpanding.expandingOverBackcover,
    height: bookBaseSize.height + coverPageBleedHeight + coverExpandingHeight
  };
};
 ...
 * @param {object} coverExpandingSize - 封面的延边. 结构为: {left, right, top, bottom}
 */
export const getInnerRenderSize = (bookBaseSize, paddings, coverExpandingSize) => {
  // 白边大小
  ...
  return {
    width: (bookBaseSize.width * 2) + coverExpandingWidth + paddingsWidth,
    height: bookBaseSize.height + coverExpandingHeight + paddingsHeight
  };
};
 ...
 * @param {Number} addedSheetNumber   - 新增的sheet数量, 一个sheet包含两个pages.
 */
export const getCoverRenderSize = (bookBaseSize,
  coverExpandingSize,
  paddings,
  spineWidth,
  addedSheetNumber = 0) => {
  ...
  // 书脊宽
  const totalSpineWidth = spineWidth.baseValue +
    (spineWidth.addtionalValue * addedSheetNumber);
  ...
  // render的宽高.
  const width = (bookBaseSize.width * 2) + coverPaddingsWidth +
    coverExpandingWidth + totalSpineWidth;
  const height = bookBaseSize.height + coverPaddingHeight +
    coverExpandingHeight;
...
export const getSpainSize = (allSpreads, parameters) => {
  ...
  if (allSpreads && allSpreads.length &&
    parameters && parameters.sheetNumberRange) {
    const {
      spineWidth,
      sheetNumberRange
    } = parameters;
...
const getSpreadSize = (project, isCover = true) => {
  ...
  if (isCover) {
    size.width = project.getIn(['cover', 'width']);
    size.height = project.getIn(['cover', 'height']);
  } else {
    ...
    if (pages.size > 2) {
      ...
      if (firstPage.get('type') === pageTypes.sheet) {
        ...
        size.width = (firstPage.get('width') + secondPage.get('width')) - (firstPage.getIn(['bleed', 'right']) + secondPage.getIn(['bleed', 'left']));
        size.height = firstPage.get('height');
      }
    }
  }
...
export const getRenderCoverSheetSize = (coverSpreadSize, parameters, ratio = 0) => {
  const size = {
    container: {
      width: 0,
      height: 0
  ...
  const bleedLeft = parameters ? parameters.getIn(['coverPageBleed', 'left']) : 0;
  const bleedRight = parameters ? parameters.getIn(['coverPageBleed', 'right']) : 0;
  const bleedTop = parameters ? parameters.getIn(['coverPageBleed', 'top']) : 0;
  const bleedBottom = parameters ? parameters.getIn(['coverPageBleed', 'bottom']) : 0;

  // 渲染book时, cover sheet的实际大小, 所有元素都是基于spread来定位.
  size.container.width = Math.ceil((coverSpreadSize.width) * ratio);
  size.container.height = Math.ceil((size.container.width * coverSpreadSize.height) / coverSpreadSize.width);

  // 渲染book时, 看到的sheet的实际大小, 其中不包括出血.
  const spreadWithWithoutBleed = (coverSpreadSize.width - (bleedLeft + bleedRight));
  const spreadHeightWithoutBleed = (coverSpreadSize.height - (bleedTop + bleedBottom));

  size.sheet.width = Math.ceil(spreadWithWithoutBleed * ratio);
  size.sheet.height = Math.ceil((size.sheet.width * spreadHeightWithoutBleed) / spreadWithWithoutBleed);
...
const getRenderInnerSheetSize = (innerSpreadSize, parameters, ratio = 0) => {
  ...
  const bleedLeft = parameters ? parameters.getIn(['innerPageBleed', 'left']) : 0;
  const bleedRight = parameters ? parameters.getIn(['innerPageBleed', 'right']) : 0;
  const bleedTop = parameters ? parameters.getIn(['innerPageBleed', 'top']) : 0;
  const bleedBottom = parameters ? parameters.getIn(['innerPageBleed', 'bottom']) : 0;
  ...
  // 渲染book时, cover sheet的实际大小, 所有元素都是基于spread来定位.
  size.container.width = Math.ceil((innerSpreadSize.width) * ratio);
  size.container.height = Math.ceil((size.container.width * innerSpreadSize.height) / innerSpreadSize.width);

  // 渲染book时, 看到的sheet的实际大小, 其中不包括出血.
  const innerSpreadWidthWithoutBleed = (innerSpreadSize.width - (bleedLeft + bleedRight));
  const innerSpreadHeightWithoutBleed = (innerSpreadSize.height - (bleedTop + bleedBottom));
  size.sheet.width = Math.ceil(innerSpreadWidthWithoutBleed * ratio);
  size.sheet.height = Math.ceil((size.sheet.width * innerSpreadHeightWithoutBleed) / innerSpreadWidthWithoutBleed);
 ...
 */
export const getRenderSize = (project, ratios, spineSize, spainExpanding, parameters, materials) => {
  // 获取cover spread的大小.
  const coverSpreadSize = getSpreadSize(project, true);
  const innerSpreadSize = getSpreadSize(project, false);

  const coverWorkspaceSize = getWorkspaceSize(coverSpreadSize, ratios.coverWorkspace);
  const innerWorkspaceSize = getWorkspaceSize(innerSpreadSize, ratios.innerWorkspace);

  const renderCoverSize = !ratios.coverRenderWidth ? coverWorkspaceSize : {
  ...
  const renderInnerSize = !ratios.innerRenderWidth ? innerWorkspaceSize : {
  ...
  let renderSpainWidthWithoutBleed = Math.ceil((spineSize.width - (spineSize.bleed.left + spineSize.bleed.right)) * ratios.coverWorkspace);

  // 渲染book时, cover sheet的实际大小: 整个spread的大小减去出血.
  const renderCoverSheetSize = getRenderCoverSheetSize(coverSpreadSize, parameters, ratios.coverWorkspace);

  // 渲染book时, inner sheet的实际大小: 整个inner spread的大小 - 出血 + 延边.
  const renderInnerSheetSize = getRenderInnerSheetSize(innerSpreadSize, parameters, ratios.innerWorkspace);

  // 校正rendersize, 由于cover和inner的出血不一样, 导致在workspace尺寸相同的情况下.
  // 渲染出来的的效果图的尺寸就不一样. 为了使cover和内页的高度一样, 我们以内页的高作为基础.
  // 对内页计算出来的尺寸做一次校正.
  if (renderCoverSheetSize.sheet.height &&
    ...
    // 校正cover渲染尺寸.
    renderCoverSheetSize.sheet.height = renderInnerSheetSize.sheet.height;
    renderCoverSheetSize.sheet.width = (oldCoverWidth * renderCoverSheetSize.sheet.height) / oldCoverHeight;
    ...
    renderCoverSheetSize.container.width = (oldContainerWidth * renderCoverSheetSize.sheet.width ) / oldCoverWidth;
    renderCoverSheetSize.container.height = (oldContainerHeight * renderCoverSheetSize.sheet.height ) / oldCoverHeight;
    ...
    coverWorkspaceSize.width = (oldCoverWorkspaceWidth * renderCoverSheetSize.sheet.width ) / oldCoverWidth;
    coverWorkspaceSize.height = (oldCoverWorkspaceHeight * renderCoverSheetSize.sheet.height ) / oldCoverHeight;

    // sheet渲染尺寸校正后, 也需要校正一下renderCoverSize的尺寸.
    renderCoverSize.width = Math.ceil(coverWorkspaceSize.width * ratios.coverRenderWidth);
    renderCoverSize.height = Math.ceil(coverWorkspaceSize.height * ratios.coverRenderHeight);

    // renderSpainWidth
    renderSpainWidth = (renderSpainWidth * renderCoverSheetSize.sheet.width ) / oldCoverWidth;
    renderSpainWidthWithoutBleed = (renderSpainWidthWithoutBleed * renderCoverSheetSize.sheet.width ) / oldCoverWidth;
  }
...
export const getRenderPosition = (size, ratios) => {
  ...
  const obj = {
    cover: {
      render: {
        top: Math.ceil(size.coverWorkspaceSize.height * ratio.coverRenderPaddingTop),
        left: Math.ceil(size.coverWorkspaceSize.width * ratio.coverRenderPaddingLeft),
        outTop: Math.ceil(size.coverWorkspaceSize.height * ratio.coverRenderOutPaddingTop),
        outLeft: Math.ceil(size.coverWorkspaceSize.width * ratio.coverRenderOutPaddingLeft)
      },
      sheet: {
        top: Math.ceil(size.coverWorkspaceSize.height * ratio.coverSheetPaddingTop),
        left: Math.ceil(size.coverWorkspaceSize.width * ratio.coverSheetPaddingLeft),
      }
    },
    inner: {
      render: {
        top: Math.ceil(size.innerWorkspaceSize.height * ratio.innerRenderPaddingTop),
        left: Math.ceil(size.innerWorkspaceSize.width * ratio.innerRenderPaddingLeft),
        outTop: Math.ceil(size.innerWorkspaceSize.height * ratio.innerRenderOutPaddingTop),
        outLeft: Math.ceil(size.innerWorkspaceSize.width * ratio.innerRenderOutPaddingLeft),
      },
      sheet: {
        top: Math.ceil(size.innerWorkspaceSize.height * ratio.innerSheetPaddingTop),
        left: Math.ceil(size.innerWorkspaceSize.width * ratio.innerSheetPaddingLeft),
      }
    }
  };
...
export const convertCoverSummaryPages = (coverSpread, settings, pagination) => {
  ...
  if (containers) {
    // 是否包含full类型的page.
    const fullPage = containers.find(page => page.get('type') === pageTypes.full);
    const frontPage = containers.find(page => page.get('type') === pageTypes.front);
    const backPage = containers.find(page => page.get('type') === pageTypes.back);
    ...
    if (fullPage) {
      pages = pages.push(fullPage.merge(draggable, {
        ...
        enabled: checkIsEnablePage(totalOfSheets, 0, 0, productType, coverType, true)
      }));
    }

    // 如果包含backPage, 就设置backPage的offset的left为0.
    if (backPage) {
      pages = pages.push(backPage.merge(draggable, {
        bgColor: 'transparent',
    ...
    if (spinePage) {
      pages = pages.push(spinePage.merge(draggable, {
        bgColor: 'transparent',
        ...
        enabled: checkIsEnablePage(totalOfSheets, 0, 1, productType, coverType, true)
      }));
    }

    // 如果包含frontPage, 就设置frontPage的offset的left为:左半页+书脊宽.
    if (frontPage) {
      pages = pages.push(frontPage.merge(draggable, {
        ...
        enabled: checkIsEnablePage(totalOfSheets, 0, 2, productType, coverType, true)
      }));
    }
  }
 ...
 */
export const getRenderPaginationSpread = (allPages, coverSpread, allElements, allDecorations, allImages, settings, pagination, onlyCover = false) => {
  const productType = get(settings, 'spec.product');
  const coverType = get(settings, 'spec.cover');
  let summary = Immutable.Map({
    isCover: true,
  ...
  if (sheetIndex === 0) {
    ...
    pages = convertSheetPages(allPages, leftIndex, rightIndex, productType, coverType, pagination.total, sheetIndex);
  ...
  if (allImages && allImages.size) {
    elements.forEach((element, key) => {
      ...
      if (encImgId) {
        const newImage = allImages.find(image => image.get('encImgId') === encImgId);
        if (newImage) {
  ...
  if (!summary.get('isCover')) {
    shadow = getShadowData(pages, settings);
  }
...
export const computedSizeForSpecialView = (size, ratioData, parameters, viewWith = 0) => {
  // 重新计算在arrange page上小尺寸的ratio.
  const ratios = computedRatioForSpecialView(size, ratioData, viewWith);
  ...
  // 渲染书脊时的实际宽度: 书脊宽度 + 书脊压线.
  // const renderSpainWidth = (spineSize.width + spainExpanding.expandingOverBackcover + spainExpanding.expandingOverFrontcover) * ratios.coverWorkspace;
  let renderSpainWidth = Math.ceil(size.spineSize.width * ratios.coverWorkspace);
  let renderSpainWidthWithoutBleed = Math.ceil((size.spineSize.width - (size.spineSize.bleed.right + size.spineSize.bleed.left)) * ratios.coverWorkspace);

  const spainExpanding = {
    expandingOverBackcover: Math.ceil(size.spainExpanding.expandingOverBackcover * ratios.coverWorkspace),
    expandingOverFrontcover: Math.ceil(size.spainExpanding.expandingOverFrontcover * ratios.coverWorkspace)
  };
  ...
  if (renderCoverSheetSize.sheet.height &&
    ...
    renderSpainWidth = (renderSpainWidth * renderCoverSheetSize.sheet.width ) / oldCoverWidth;
    renderSpainWidthWithoutBleed = (renderSpainWidthWithoutBleed * renderCoverSheetSize.sheet.width ) / oldCoverWidth;
    spainExpanding.expandingOverBackcover = (spainExpanding.expandingOverBackcover * renderCoverSheetSize.sheet.width ) / oldCoverWidth;
    spainExpanding.expandingOverFrontcover = (spainExpanding.expandingOverFrontcover * renderCoverSheetSize.sheet.width ) / oldCoverWidth;
  }
 ...
 * - onlyCover: 是否仅仅计算封面的页面数据.
 */
export const getRenderAllSpreads = (allPages, coverSpread, allElements, allDecorations, allImages, settings) => {
  // 计算sheet的数量: 加上一个封面.
  ...
  for (let i = 0; i < countOfSheets; i++) {
    const sheet = getRenderPaginationSpread(allPages, coverSpread, allElements, allDecorations, allImages, settings, {
      sheetIndex: i,
      pageStep,
      total: allPages.size / 2
    });
