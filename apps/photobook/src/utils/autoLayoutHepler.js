 ...
 */
export const recomputedElementOptionsWithImage = (image, templateElement, page) => {
  // 获取页面的宽高.
 ...
 */
export const recomputedElementOptions = (allImages, pageElement, templateElement, page) => {
  // 获取图片的原始大小.
  const image = getImage(pageElement, allImages);

  return recomputedElementOptionsWithImage(image, templateElement, page);
};
 ...
 */
export const updateElementsByTemplate = (page, pageElements, images, template, isHalfCover = false) => {
  // 给页面上所有的元素对象根据encImgId进行排序.
  // 将含有图片的elements移到前面.
  const sortedPageElements = pageElements.sort((first, second) => second.get('encImgId') > first.get('encImgId'));
  const textElements = sortedPageElements.filter(element => element.get('type') === elementTypes.text);
  const photoElements = sortedPageElements.filter(element => element.get('type') === elementTypes.photo);
  const decorationElements = sortedPageElements.filter(element => element.get('type') === elementTypes.decoration);
    ...
    switch(templateElement.type){
      case elementTypes.photo:{
        if(++photosApplyedTemplate <= photoElements.length){
          ...
          let newElement = pageElement.merge(recomputedElementOptions(images, pageElement, newTemplateElement, page));
          ...
          const newElement = new Element(recomputedElementOptions(images, Map({}), newTemplateElement, page));
          newElements = newElements.push(Map(merge(newElement, {id: guid()})));
        }
        break;
      }
      case elementTypes.text:{
        if (++textsApplyedTemplate <= textElements.length) {
          ...
          const newElement = pageElement.merge(recomputedTextElementOptions(newTemplateElement, page));
          newElements = newElements.push(newElement);
        } else {
          let newTemplateElement = templateElement;
          ...
          const newElement = recomputedTextElementOptions(newTemplateElement, page);
          newElements = newElements.push(Map(merge(newElement, {id: guid()})));
        }
        break;
      }
 ...
 * @return {[type]}          [description]
 */
export const createNewElementsByTemplate = (page, images, template, ratio, isHalfCover = false) => {
  const newElements = [];
  ...
  const sortedImages = images.sort((first, second) => {
    return first.get('width') / first.get('height') > second.get('width') / second.get('height');
  });
  ...
  if (textInTemplate) {
    ...
    newElements.push(Map(merge({}, newTextElement, {
      id: guid(),
      type: elementTypes.text
    })));
  }
  ...
  sortedTemplateElements.forEach((templateElement, index) => {
    if(sortedImages.size > index && templateElement.type === elementTypes.photo){
      const image = sortedImages.get(index).toJS();
      const baseElement = new Element(recomputedElementOptionsWithImage(image, templateElement, page));

      newElements.push(Map(merge({}, baseElement, {
        id: guid(),
