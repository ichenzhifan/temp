...
export const filterOptionMapByPro = (filteredOptionMap, proUserOptions) => {
  let resMap = merge({}, filteredOptionMap);
  for (const key in resMap) {
...
export const resetSizeMap = (sizeMap) => {
  ...
  const newSizeMap = squareSizeMap.concat(landscapeSizeMap).concat(portraitSizeMap);
...
export const settingConstructor = (that, setting, oldSetting) => {
  let newOptionsMap;
  let disableOptionMap;
  const { env } = that.props;
  const isProCustomer = env.userInfo && env.userInfo.get('isProCustomer');
  if (configurableOptionArray && allOptionMap && variableArray) {
    newOptionsMap = projectParser.getAvailableOptionMap(setting, configurableOptionArray, allOptionMap);
    disableOptionMap = projectParser.getDisableOptionMap(setting, disableOptionArray);
  }
  //  该项为 disableOptionMap 的过滤时的项目保留且置为 disable 属性提供依据。
  const projectSetting = oldSetting || setting;
  let filteredOptionMap = filterOptionMap(newOptionsMap, disableOptionMap, projectSetting);
  ...
  return {
    setting,
    selectMap: {
      productMap: filteredOptionMap ? resetOptionMap(filteredOptionMap.product) : [],
      coverMap: filteredOptionMap ? resetOptionMap(filteredOptionMap.cover) : [],
      colorMap: filteredOptionMap ? resetOptionMap(filteredOptionMap.leatherColor) : [],
      sizeMap: filteredOptionMap ? resetSizeMap(filteredOptionMap.size) : [],
      paperMap: filteredOptionMap ? resetOptionMap(filteredOptionMap.paper) : [],
      paperThicknessMap: filteredOptionMap ? resetOptionMap(filteredOptionMap.paperThickness) : [],
      gildingMap: filteredOptionMap ? resetOptionMap(filteredOptionMap.gilding) : []
    },
    settingLabels: {
...
export const changeSetting = (that, param, done) => {
  const newSettingObj = projectParser.getNewProjectSetting(
    that.state.setting, param, configurableOptionArray
  );
  const newSettingsState = that.settingConstructor(newSettingObj, that.props.bookOptionsData.getIn(['settings', 'spec']).toJS());
...
export const cancelSetting = (that, done) => {
  ...
  const settingObj = project.get('setting').toJS();
