    ...
    success: function (result) {
      if (result) {
        ...
        for (let i = 0; i < idCount; i++) {
          ...
          if (['image/jpeg','image/jpg','image/png','image/x-png'].indexOf(files[i].type) !== -1) {
            const file = files[i];
                ...
                if (res) {
                  ...
                  if (res.indexOf('state="success"') !== -1) {
                    ...
                    if (shouldAutoAddPhotoToCanvas && shouldAutoAddPhotoToCanvas.status && !locked) {
                      const { spreadId, targetHeight, targetWidth } = shouldAutoAddPhotoToCanvas;
                      locked = true;
            ...
            dispatch({
              ...
              fields: {
                status: FAIL,
                percent: "Only .jpg .jpeg and .png files are supported",
                info: "Only .jpg .jpeg and .png files are supported",
                file: null
              }
            });
            //将上传失败的放到第一个
